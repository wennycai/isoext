...
def chain_iso(kernelPointsIn, E):
    ...
    while kernelPoints:
        ...
        philist.append(phi)
    print("The runtime of computing isogenies from kernel points is",cputime(t),"s")
...
class Deuring_Context:
    ...
    @cached_method
    def twistE0_new(self, Fbig):
        r"""
        Given an extension field F_{p^2k}, returns a quadratic twist \widetilde{E_0} of E_0 over F_{p^2k}.
        The twist has its own isomorphism \widetilde{E_0} -> O_0
        """
        Ek = self.E0.change_ring(Fbig)
        while True:
            D = Fbig.random_element()
            if not D.is_square():
                break
        ...
        E = EllipticCurve([a*tmp_a**2, b*tmp_a**3])
        assert E.is_isomorphic(Ek.quadratic_twist())
        E.twist_D = D       #FIXME adding this attribute is quite a hack
    ...
    def com_minipoly_k_P(self, E, l):
        r"""
        Given a curve E, an order l, output a minimal polynomial of x(P), where P is a point of order l.
        """
        F2 = E.base_field()
        O0_alg = self.O0.quaternion_algebra()
        i,j,k = O0_alg.gens()
        q = ZZ(-i**2)
        QF = BinaryQF([1,0,q])
    ...
    def com_minipoly_k_Q(self, E, phi_a,tau_b):
        r"""
        Given a curve E, an order l, the rational map of [a] and iota*[b],
        output a minimal polynomial of x(Q), where Q is a point of order l linear dependent to P.
        """
        F2 = E.base_field()
        # Step 1. Construct map phi = [a]-\tau[b]
        # Elliptic curve point addition in affine coordinatess
        lam = (phi_a[1]+tau_b[1])/(phi_a[0]-tau_b[0])
        phi_x = lam**2-E.a2()-phi_a[0]-tau_b[0]
        ...
        x,y,z = R.gens()
        #print("Construction is finished")
        # Step 2. Take the denominator of phi as psi.
        #print("Start to extract psi")
        g = [y**2-z]
        tmp_0 = R(phi_x.numerator())
        tmp_1 = R(phi_x.denominator())
        ...
        # fq = tmp[len(tmp)-1][0]
        assert psi_x.is_square()
        fq = psi_x.sqrt() #TODO: Use FFT to get \tilede(O)(l) complexity.
        #print("Extraction is finished")
        return fq

    def construct_ext_P(self, fp):
        r"""
        Given an irreducible polynomial fp of degree $k$, output the extension field Fp^2k with modulus equals to fp.
        """
        F2 = self.E0.base_field()
        Fp = F2.base_ring()
    ...
    def construct_ext_Q(self, fq):
        r"""
        Given an irreducible polynomial fq of degree $k$, output the extension field Fp^2k with modulus equals to fq.
        """
        F2 = self.E0.base_field()
        Fp = F2.base_ring()
        # say this is the polynomial over Fq with which you want to take the extension
    ...
    def IdealToIsogenyGens2(self, I, specificTorsion=0):
        r"""
        New algortim for computing kerner points.
        ...
        for (l,e) in N.factor():
            lval = d.valuation(l)  # point divisions
            if l %4 == 1 and e+lval ==1 and self.facToExt[l]==(l-1)//2:
                print("Using new method when l = ",l,"and k = ",(l-1)//2)
                t = cputime()
                Pl,phi_a,tau_b = self.genTorsionBasisP_1mod4_one(self.E0, l)
                R,twD = self.evalIdealElt(a.conjugate(), Pl)
                if not R:
                    print("We need to Generate Q in new method")
                    Ql = self.genTorsionBasisQ_1mod4_one(self.E0, l, phi_a, tau_b)
                    R,twD = self.evalIdealElt(a.conjugate(), Ql)
                print("Using new method requires", cputime(t),"s")
                t = cputime()
                ...
                if not l**(e-1) * R:
                    ...
                    R,twD = self.evalIdealElt(l**lval * a.conjugate(), Q)
                print("Using new method (without torsion basis) requires", cputime(t),"s")
            else:
                P, Pl = self.genTorsionBasisP(self.E0, l, e+lval)
                R,twD = self.evalIdealElt(l**lval * a.conjugate(), P)
                if not l**(e-1) * R:
    ...
    def IdealToIsogenyGens3(self, I, specificTorsion=0):
        r"""
        If p = 1 mod 4, we should use this version.
        Given a left O0-ideal I, returns {P_1, .., P_n} such that ker phi_I = <P_1, .., P_n>
        """
        kerGens = []
        a = DecompAlphaN(I)
    ...
    def genTorsionBasisQ_1mod4_one(self, E, l, phi_a, tau_b):
        """
        Given a curve E, prime l, output a l-torsion point Q on E.
        """
        assert E.j_invariant() == 1728

        # Compute minimal polynomial of degree k for xQ, satisfies l|p^k+1.
        fq = self.com_minipoly_k_Q(E,phi_a,tau_b)

        # Construct extension field Fp^2k with fq
        Fbig_q = self.construct_ext_Q(fq)

        assert E is self.E0
        # If l = 1 mod 4, the l-torsion point is awalys on the twisted curve.
        # Compute the twisted curve
        E, tmp_a = self.twistE0_new_Q(Fbig_q)
        xQ = Fbig_q.gen()
        a4 = E.a4()
        # We need to map the point
        xQ = xQ*tmp_a
    ...
    def IdealToIsogeny(self, I, specificTorsion=0):
        ...
        print("The runtime of computing kernel points (original) is",cputime(t),"s")
        verbose("Evaluating isogeny...")
        return chain_iso(kerGens, self.E0)

    def IdealToIsogeny_new(self, I, specificTorsion=0):
        r"""
        An improved version for computing the corresponding isogeny phi_I of a left O0-ideal I.
        """
        t = cputime()
        if self.p %4 ==1:
            kerGens = self.IdealToIsogenyGens3(I, specificTorsion=specificTorsion)
        else:
            kerGens = self.IdealToIsogenyGens2(I, specificTorsion=specificTorsion)
        print("The runtime of computing kernel points (without torsion)is",cputime(t),"s")
        verbose("Evaluating isogeny...")
        return chain_iso(kerGens, self.E0)
...
def constructive_deuring(I, E0, iota, variant=None):
    ...
    print("The runtime of KLPT is ",cputime(t),"s")
...
def constructive_deuring_2(I, E0, iota, variant=None):
    r"""
    Given a curve E0, where End(E0) is isomorphic to O0 (a special
    p-extremal maximal order in the quaternion algebra B = <1,i,j,k>),
    the endomorphism iota corresponding to i, and a left O0 ideal I,
    ...
    KLPT_ctx = KLPT_Context(O0.quaternion_algebra())
    t = cputime()
    J, facToExt, T, S, f = KLPT_ctx.KLPT_2(I, variant=variant)
    print("The runtime of KLPT is ",cputime(t),"s")
    print(f'norm(J) = {factor(J.norm())}')
    verbose(f'{J=}')
    assert gcd(J.left_order().unit_ideal().basis_matrix().solve_left(J.basis_matrix()).list()) == 1, 'non-cyclic ideal'
...
def constructive_deuring_new(I, E0, iota, variant=None):
    r"""
    An improved version to compute constructive deuring.
    ...
    if S:
        ...
        print("Using one-shot ideal-isogeny translation")
        phi_I = Deuring_ctx.IdealToIsogeny_new(J)
    return phi_I.codomain(), phi_I, Deuring_ctx

def constructive_deuring_new_2(I, E0, iota, variant=None):
    r"""
    An improved version to compute constructive deuring.
