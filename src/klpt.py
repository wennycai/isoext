...
class KLPT_Context:
        ...
        while not nu:
            ...
            if not T:
                ...
                if variant == 'simple':
                    ...
                    S = None
                    f = None
                    print("The runtime of cost model is ",cputime(t),"s")
                elif variant == 'slide':
    ...
    def KLPT_2(self, I, T=None, returnElem=False, variant=None):
        r"""
        Given an integral ideal I, and a target norm T,
        outputs an equivalent left ideal J~I, with nrd(J) dividing T.
        ...
        while not nu:
            ...
            N = ZZ(II.norm())
            if not T:
                from .cost import choose_torsion_2
                verbose("Finding output norm T")
                if variant == 'simple':
                    t = cputime()
                    T, facToExt = choose_torsion_2(p, self.q, N, max(p*N**4, p**3))
                    fac = list(factor(T))
                    S = None
                    f = None
                    print("The runtime of cost model is ",cputime(t),"s")
                elif variant == 'slide':
                    T, facToExt = choose_torsion_2(p, self.q, N, p**(4.2/2), newconst=(0.31, 15*1.17, 15*0.46, 15*0.01))
                    Tfactors = [l**e for (l,e) in factor(T)]
                    Tfactors.sort()
                    S = 1
                    for le in Tfactors:
